...
// Cache simple du mapping ringoverUserId → firebaseUid
...
async function getRingoverUserMap() {
  ...
  try {
    ...
    if (Object.keys(map).length === 0 && phoneSnap.size === 1) {
      const credSnap = await db.collection('_config').doc('telco_credentials').get();
      const cred = credSnap.data()?.ringover;
      if (cred?.userId) map[String(cred.userId)] = phoneSnap.docs[0].data().assignedTo;
    }
  } catch (_) {}
  _userMapCache = map;
  return map;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Récupérer le Firebase UID associé au user Ringover (pour visibilité dans le dialer)
    let firebaseUid = null;
    if (d.user_id) {
      const map = await getRingoverUserMap();
      firebaseUid = map[String(d.user_id)] || null;
    }
    const userIdExtra = firebaseUid ? { userId: firebaseUid, provider: 'ringover' } : { provider: 'ringover' };
