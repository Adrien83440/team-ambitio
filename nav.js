 ...
 * Gère la sidebar avec accès par rôle (coach / sales / admin)
 * Design : violet dégradé pour coaching, rouge profond pour sales
 */
...
(function () {
  ...
  const MODULES = {
    coach: [
      { id: 'dashboard',   icon: '◈',  label: 'Vue d\'ensemble',  href: 'coaching.html' },
      { id: 'clients',     icon: '◎',  label: 'Mes clients',       href: 'coaching.html#clients' },
      { id: 'sessions',    icon: '◷',  label: 'Sessions',          href: 'coaching.html#sessions' },
      { id: 'progression', icon: '▲',  label: 'Progression',       href: 'coaching.html#progression' },
      { id: 'ressources',  icon: '❖',  label: 'Ressources',        href: '#', badge: 'Bientôt' },
      { id: 'messagerie',  icon: '◉',  label: 'Messagerie',        href: '#', badge: '3' },
    ],
    sales: [
      { id: 'dashboard',   icon: '◈',  label: 'Dashboard',         href: 'sales-dashboard.html' },
      { id: 'saisie',      icon: '◎',  label: 'Saisie',            href: 'sales-saisie.html' },
    ...
    admin: [
      ...
      // ── SALES ──
      { id: 'dashboard-s', icon: '◆',  label: 'Dashboard',         href: 'sales-dashboard.html', section: 'Sales' },
      { id: 'saisie',      icon: '◎',  label: 'Saisie',            href: 'sales-saisie.html',    section: 'Sales' },
      ...
      { id: 'equipe-s',    icon: '◉',  label: 'Équipe',            href: 'sales-equipe.html',    section: 'Sales' },
      ...
      { id: 'retargeting', icon: '◎',  label: 'Retargeting',       href: 'sales-retargeting.html', section: 'Sales' },
      // ── OUTILS ──
  ...
  const THEMES = {
    coach: {
      ...
      roleColor: '#a78bfa',
      roleBg: 'rgba(167,139,250,0.12)',
      roleBorder: 'rgba(167,139,250,0.25)',
    },
    sales: {
      label: 'Espace Sales',
      emoji: '📈',
      ...
      roleColor: '#fca5a5',
      roleBg: 'rgba(252,165,165,0.12)',
      roleBorder: 'rgba(252,165,165,0.25)',
    },
    admin: {
      ...
      roleColor: '#fbbf24',
      roleBg: 'rgba(251,191,36,0.12)',
      roleBorder: 'rgba(251,191,36,0.25)',
    },
  };
  ...
  style.textContent = `
    ...
    .nav-role-badge {
      ...
      padding: 8px 10px;
      border-radius: 10px;
      background: var(--nav-role-bg);
      border: 1px solid var(--nav-role-border);
      overflow: hidden;
      transition: all 0.2s;
    }
    .nav-role-icon {
    ...
    #ambitio-sidebar.collapsed .nav-section-label {
      opacity: 0;
      pointer-events: none;
    }
    #ambitio-sidebar.collapsed .nav-role-badge {
      justify-content: center;
    }
    #ambitio-sidebar.collapsed .nav-collapse-btn {
      margin-left: 0;
    }
    #ambitio-sidebar.collapsed .nav-header {
      justify-content: center;
    ...
    .nav-avatar {
      ...
      border-radius: 9px;
      background: var(--nav-accent-glow);
      border: 1.5px solid var(--nav-role-border);
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 13px;
    ...
    #ambitio-sidebar.collapsed .nav-profile-caret {
      opacity: 0;
      pointer-events: none;
    }
    #ambitio-sidebar.collapsed .nav-profile-btn {
      justify-content: center;
      padding: 9px;
    }
    ...
    .pm-avatar-big {
      ...
      border-radius: 16px;
      background: var(--nav-accent-glow);
      border: 2px solid var(--nav-role-border);
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 26px;
    ...
    .pm-field textarea::placeholder { color: rgba(255,255,255,0.2); }
  ...
  function buildSidebar() {
    ...
    const roleLabels = { coach: '🎓 Coach', sales: '📈 Commercial', admin: '👑 Admin' };
      ...
      <div class="nav-role-section">
        <div class="nav-role-badge">
          ...
          <div class="nav-role-text">
            ...
            <div class="nav-role-name">${roleLabels[role] || role}</div>
          </div>
        </div>
      </div>
  ...
  function openProfileModal() {
    ...
    const roleLabels = { coach: '🎓 Coach', sales: '📈 Commercial', admin: '👑 Administrateur' };
    ...
    backdrop.innerHTML = `
      <div class="profile-modal">
        ...
        <div class="pm-avatar-section">
          <div class="pm-avatar-big" style="background:${theme.accentGlow};border-color:${theme.roleBorder}">${user.initials}</div>
          <div class="pm-avatar-info">
            <div class="pm-avatar-name">${user.name}</div>
            <div class="pm-avatar-role">${roleLabels[role] || role}</div>
          </div>
        </div>
        <div class="pm-body">
          ...
          <div class="pm-row">
            <div class="pm-field">
              <label>Prénom</label>
              <input type="text" id="pmFirstname" placeholder="Prénom" value="${saved.firstname || ''}"/>
            </div>
            <div class="pm-field">
              <label>Nom</label>
              <input type="text" id="pmLastname" placeholder="Nom" value="${saved.lastname || ''}"/>
            </div>
          </div>
          <div class="pm-field">
            <label>Email professionnel</label>
            <input type="email" id="pmEmail" placeholder="prenom@ambitiocorp.com" value="${saved.email || user.email}"/>
          </div>
          <div class="pm-field">
            <label>Téléphone</label>
            <input type="tel" id="pmPhone" placeholder="+33 6 00 00 00 00" value="${saved.phone || ''}"/>
          </div>
          ...
          <div class="pm-row">
            <div class="pm-field">
              <label>Titre</label>
              <input type="text" id="pmTitle" placeholder="Coach senior / BDR..." value="${saved.title || ''}"/>
            </div>
            <div class="pm-field">
              <label>Département</label>
              <select id="pmDept">
                <option value="">Choisir...</option>
                <option value="coaching" ${saved.dept === 'coaching' ? 'selected' : ''}>Coaching</option>
                <option value="sales" ${saved.dept === 'sales' ? 'selected' : ''}>Sales</option>
                <option value="ops" ${saved.dept === 'ops' ? 'selected' : ''}>Ops</option>
                <option value="marketing" ${saved.dept === 'marketing' ? 'selected' : ''}>Marketing</option>
              </select>
            </div>
          </div>
          <div class="pm-field">
            <label>LinkedIn</label>
            <input type="url" id="pmLinkedin" placeholder="https://linkedin.com/in/..." value="${saved.linkedin || ''}"/>
          </div>

          <div class="pm-section-title">Préférences</div>
          <div class="pm-field">
            <label>Bio courte</label>
            <textarea id="pmBio" rows="3" placeholder="Quelques mots sur toi...">${saved.bio || ''}</textarea>
          </div>
          <div class="pm-row">
            <div class="pm-field">
              <label>Langue</label>
          ...
          <div class="pm-actions">
            ...
            <button class="pm-btn pm-btn-primary" id="pmSave" style="color:#0f0f1a">💾 Sauvegarder</button>
          </div>
    ...
    // Events
    document.getElementById('pmClose').addEventListener('click', () => backdrop.remove());
    document.getElementById('pmCancel').addEventListener('click', () => backdrop.remove());
    backdrop.addEventListener('click', e => { if (e.target === backdrop) backdrop.remove(); });

    document.getElementById('pmSave').addEventListener('click', () => {
      const profile = {
        firstname: document.getElementById('pmFirstname').value,
        lastname:  document.getElementById('pmLastname').value,
        ...
        dept:      document.getElementById('pmDept').value,
        linkedin:  document.getElementById('pmLinkedin').value,
        bio:       document.getElementById('pmBio').value,
        lang:      document.getElementById('pmLang').value,
        tz:        document.getElementById('pmTz').value,
      };
      localStorage.setItem('ambitio_profile', JSON.stringify(profile));
      const toast = document.getElementById('pmToast');
      toast.classList.add('show');
      setTimeout(() => { toast.classList.remove('show'); setTimeout(() => backdrop.remove(), 300); }, 1800);
    });

    document.getElementById('pmLogout').addEventListener('click', async () => {
      if (window._signOut && window._auth) {
        await window._signOut(window._auth);
      }
      localStorage.removeItem('ambitio_role');
      localStorage.removeItem('ambitio_name');
      localStorage.removeItem('ambitio_email');
  ...
  window.AmbitioNav = {
    ...
    rebuild() {
      const el = document.getElementById('ambitio-sidebar');
      if (el) el.remove();
