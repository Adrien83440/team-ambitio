...
var STAGES=[
  ...
  /* Refonte 14/07 : statut unique « Closing » — une seule colonne/pastille.
     Les stages internes closed_won_setting / closed_won_self restent en base
     (rapports SB/NB, funnel, commissions) mais sont cachés (hidden:1) et
     s'affichent « Closing ». La colonne/pastille « closing » (virtual:1)
     ouvre les CARTES du close (close-wizard.js) qui décident SB/NB. */
  {key:'closing',label:'Closing',color:'#10b981',section:'Closing',virtual:1},
  {key:'closed_won_setting',label:'Closing',color:'#10b981',section:'Closing',hidden:1},
  {key:'closed_won_self',label:'Closing',color:'#059669',section:'Closing',hidden:1},
...
/* Stage effectif pour l'AFFICHAGE : les deux stages Won internes vivent dans
   la colonne unique « Closing ». Les données ne changent pas. */
function effStage(l){var s=(l&&l.stage)||'lead';return(s==='closed_won_setting'||s==='closed_won_self')?'closing':s;}
/* Ouvre les cartes du close pour un lead (colonne/pastille/sheet « Closing »). */
function openCloseWizard(lid){
  var wl=null;for(var i=0;i<allLeads.length;i++){if(allLeads[i].id===lid){wl=allLeads[i];break;}}
  if(!window.CloseWizard){toast('❌ close-wizard.js manquant');return;}
  window.CloseWizard.open({leadId:lid,lead:wl,onDone:function(res){
    var ns=(res&&res.sb)?'closed_won_self':'closed_won_setting';
    for(var j=0;j<allLeads.length;j++){if(allLeads[j].id===lid){allLeads[j].stage=ns;allLeads[j].status='client';allLeads[j].isClient=true;break;}}
    renderAll();
  }});
}
  ...
  criteria.forEach(function(c){
    ...
    list=list.filter(function(l){
      ...
      /* stage : accepte la valeur brute ET le stage effectif (les critères
         « closing » matchent closed_won_setting / closed_won_self). */
      var val2=c.field==='stage'?effStage(l):val;
      if(c.op==='eq'){var vs=cv.split(',').map(function(v){return v.trim();});return vs.indexOf(val)>=0||vs.indexOf(val2)>=0;}
      if(c.op==='neq'){var ns=cv.split(',').map(function(v){return v.trim();});return ns.indexOf(val)<0&&ns.indexOf(val2)<0;}
...
function renderPipeline(leads){
  ...
  leads.forEach(function(l){var st=effStage(l);if(counts[st]!==undefined)counts[st]++;});
...
function renderSheet(leads){
  ...
  var h='';page.forEach(function(l,idx){
    ...
    SHEET_COLS.forEach(function(col){
      ...
      else if(col.edit==='select'&&col.key==='stage'){h+='<td><select class="sheet-cell-select" data-sf="stage">';STAGES.forEach(function(s){if(s.hidden)return;h+='<option value="'+s.key+'"'+(effStage(l)===s.key?' selected':'')+'>'+esc(s.label)+'</option>';});h+='</select></td>';}
...
document.getElementById('bulkStageBtn').addEventListener('click',function(e){
  ...
  var h='';STAGES.forEach(function(s){if(s.hidden)return;h+='<div class="bulk-stage-item" data-bstage="'+s.key+'"><span style="width:8px;height:8px;border-radius:50%;background:'+s.color+';flex-shrink:0"></span>'+esc(s.label)+'</div>';});
...
document.getElementById('bulkStageDD').addEventListener('click',function(e){
  ...
  /* Un close = un contrat + des commissions : ça se valide lead par lead. */
  if(ns==='closing'){toast('🏆 Le close se valide lead par lead — ouvre le lead et clique « Closing » (cartes)');document.getElementById('bulkStageDD').classList.remove('open');return;}
...
document.getElementById('sheetBody').addEventListener('change',function(e){var el=e.target.closest('[data-sf]');if(!el)return;var tr=el.closest('tr[data-sid]');if(!tr)return;if(sheetTimer){clearTimeout(sheetTimer);sheetTimer=null;}saveSheetRow(tr);});
function saveSheetRow(tr){var lid=tr.dataset.sid,upd={};tr.querySelectorAll('[data-sf]').forEach(function(el){upd[el.dataset.sf]=el.value.trim();});
/* « Closing » choisi dans la feuille → cartes du close (le stage n'est pas
   écrit ici — le wizard s'en charge) ; les AUTRES champs de la ligne sont
   quand même sauvegardés. */
if(upd.stage==='closing'){var _l0=null;for(var z=0;z<allLeads.length;z++){if(allLeads[z].id===lid){_l0=allLeads[z];break;}}delete upd.stage;if(!_l0||effStage(_l0)!=='closing'){if(sheetTimer){clearTimeout(sheetTimer);sheetTimer=null;}openCloseWizard(lid);}}
upd.updatedAt=firebase.firestore.FieldValue.serverTimestamp();if(upd.stage&&S2S[upd.stage])upd.status=S2S[upd.stage];if(upd.stage&&(upd.stage==='closed_won_setting'||upd.stage==='closed_won_self')){upd.isClient=true;upd.clientSince=firebase.firestore.FieldValue.serverTimestamp();}for(var i=0;i<allLeads.length;i++){if(allLeads[i].id===lid){for(var k in upd){if(k!=='updatedAt')allLeads[i][k]=upd[k];}break;}}db.collection('leads').doc(lid).update(upd).then(function(){toast('✅ Sauvegardé');}).catch(function(err){toast('❌ '+err.message);});}
