...
module.exports = async (req, res) => {
  ...
  // ── Mapping ringoverUserId → firebaseUid via phone_numbers ──────────────
  ...
  try {
    ...
    phoneSnap.forEach(d => {
      const x = d.data();
      // Si phone_numbers a un champ ringoverUserId explicite, l'utiliser
      if (x.ringoverUserId && x.assignedTo) {
        ringoverUserToUid[String(x.ringoverUserId)] = x.assignedTo;
      }
    });
    // Fallback : un seul user Ringover → mapper sur le premier numéro actif
    if (Object.keys(ringoverUserToUid).length === 0 && phoneSnap.size === 1) {
      const credSnap = await db.collection('_config').doc('telco_credentials').get();
      const cred = credSnap.data()?.ringover;
      if (cred?.userId) {
        ringoverUserToUid[String(cred.userId)] = phoneSnap.docs[0].data().assignedTo;
      }
    }
    console.log('[sync-ringover] User mapping:', ringoverUserToUid);
  } catch (e) {
    console.warn('[sync-ringover] Mapping error:', e.message);
  }
  ...
  for (const call of callList) {
    try {
      ...
      if (call.user) {
        ...
        update.ringoverUserName = update.ringoverUserName.trim() || null;
        // Mapper vers le Firebase UID pour que le user voie ses appels
