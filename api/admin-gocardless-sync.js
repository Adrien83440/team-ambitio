...
async function createInvoiceForGcPayment(gcPayment, billing, systemKey, validateUrl, user) {
  ...
  try {
    ...
    for (const p of pag.items) {
      ...
      if (dryRun) {
        ...
        try {
          const created = await createInvoiceForGcPayment(p, billing, systemKey, validateUrl, user);
          if (created.skipped) {
