...
module.exports = async function(req, res) {
  ...
  try {
    ...
    if (dryRun) {
      ...
      /* Calcul aussi du nouveau nextScheduledAt pour la prévision */
      const lastChargeStrDry = payments[payments.length - 1].chargeDate;
      const lastChargeDry = new Date(lastChargeStrDry + 'T12:00:00');
      const newNextScheduledDry = new Date(lastChargeDry.getFullYear(), lastChargeDry.getMonth() + 1, lastChargeDry.getDate(), 2, 0, 0);
      const wouldComplete = sub.totalInstallments != null && ((sub.installmentsGenerated || 0) + payments.length) >= sub.totalInstallments;
      ...
      res.status(200).json({
        ...
        currentNextScheduledAt: sub.nextScheduledAt && sub.nextScheduledAt.toDate
          ...
          : null,
        wouldUpdateNextScheduledAtTo: wouldComplete ? null : newNextScheduledDry.toISOString(),
        wouldMarkCompleted: wouldComplete,
        vatRate: vatRate,
        vatType: vatType,
    ...
    /* ── Update subscription : installmentsGenerated += N + avancer nextScheduledAt ── */
    ...
    /* Avancer nextScheduledAt au mois suivant le dernier paiement rattrapé.
       Sans ça, la cron 4B verra la sub toujours overdue (nextScheduledAt
       dans le passé) avec installmentsGenerated > 0, ce qui réactivera le
       garde-fou 3j et créerait à nouveau une alerte subscription_overdue. */
    const lastChargeStr = payments[payments.length - 1].chargeDate;
    const lastCharge = new Date(lastChargeStr + 'T12:00:00');
    /* Anchor au jour du mois du chargeDate + 1 mois (cohérent avec
       subscription-generate-invoice.js qui fait cur.getMonth() + 1) */
    const newNextScheduled = new Date(lastCharge.getFullYear(), lastCharge.getMonth() + 1, lastCharge.getDate(), 2, 0, 0);
    ...
    await subRef.update({
      ...
      nextScheduledAt: newStatus === 'completed' ? null : admin.firestore.Timestamp.fromDate(newNextScheduled),
      status: newStatus,
      ...
      _previousNextScheduledAt: sub.nextScheduledAt || null,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      updatedBy: user.uid,
    });
    ...
    res.status(200).json({
      ...
      newStatus: newStatus,
      newNextScheduledAt: newStatus === 'completed' ? null : newNextScheduled.toISOString(),
    });
