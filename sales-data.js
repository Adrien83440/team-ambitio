...
function getMonthWeeks(monthLabel) {
  ...
  while (cursor <= lastDay) {
    ...
    let end;
    if (dow === 1) {
      end = new Date(cursor.getTime() + 6 * 86400000);
    } else {
      const daysToSunday = dow === 0 ? 0 : (7 - dow);
      end = new Date(cursor.getTime() + daysToSunday * 86400000);
    }
    if (end > lastDay) end = new Date(lastDay);

    weeks.push({
      id: 's' + (weeks.length + 1), label: 'S' + (weeks.length + 1),
      start: new Date(cursor), end: new Date(end),
      range: fmt(cursor) + ' → ' + fmt(end),
      short: fmtShort(cursor) + ' – ' + fmtShort(end),
      tab:   fmt(cursor) + '→' + fmt(end),
    });
